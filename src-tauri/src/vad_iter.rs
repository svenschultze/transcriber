...
const DEBUG_SPEECH_PROB: bool = false; // Set to false to reduce console output in production
...
impl VadIter {
    ...
    pub fn process(&mut self, samples: &[i16]) -> Result<(), Box<dyn std::error::Error>> {
        self.reset_states();
